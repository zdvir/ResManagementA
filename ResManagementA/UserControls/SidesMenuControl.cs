...
{
    public partial class SidesMenuControl : UserControl, MenuInterface
    {
        ...
        private void GetOrRefreshData()
        {
            for (int i = 0; i < sidesArray.Length; i++)
            {
                ...
                textBoxArray[i].Text = sidesArray[i].Stocks.ToString();
                textBoxArray[i + 6].Text = sidesArray[i].Price.ToString();
            }
        }
        ...
        private void SetNumericMaxValue()
        {
            for (int i = 0; i < numericUpDownArray.Length; i++)
                    numericUpDownArray[i].Maximum = sidesArray[i].Stocks;   
        }
        ...
        public void SetNumericValue()
        {
            ...
            foreach (Order order in orderList)
                for (int i = 0; i < sidesArray.Length; i++)
                {
                    if (order.ProductName.Equals(sidesArray[i].Name))
                    {
                        ...
                        numericUpDownArray[i].Maximum = sidesArray[i].Stocks + numericUpdateModeArray[i];
        ...
        public bool NewOrder()
        {
            ...
            Order[] orderList = new Order[sidesArray.Length];
            ...
            for (int i = 0; i < sidesArray.Length; i++)
            {
                ...
                positiveStocks = sidesArray[i].Stocks - numericValue;

                if (numericValue != 0 && positiveStocks >= 0)
                {
                    isChanged = true;
                    orderList[i] = new Order(currentTable, currentUser, sidesArray[i].Name, numericValue, sidesArray[i].Price);
                    sidesArray[i].Stocks = sidesArray[i].Stocks - numericValue;
                    dbHandler.UpdateProduct(TABLE_SIDES, sidesArray[i]);
                }
                else
                {
                    orderList[i] = null;
        ...
        public bool UpdateOrder()
        {
            ...
            for (int i = 0; i < sidesArray.Length; i++)
            {
                ...
                {
                    isChanged = true;
                    orderList[i] = new Order(currentTable, "", sidesArray[i].Name, numericValue, sidesArray[i].Price);
                }
                else
                {
                    orderList[i] = null;
                ...
                //Get the Correct Product Stock (Plus the "Before", Minus the "After")
                positiveStocks = sidesArray[i].Stocks + numericUpdateModeArray[i] - numericValue;
                if (positiveStocks >= 0)
                {
                    sidesArray[i].Stocks = positiveStocks;
                    dbHandler.UpdateProduct(TABLE_SIDES, sidesArray[i]);
                }
                else
                {
